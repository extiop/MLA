...
fn get_mla_version_from_archive(matches: &ArgMatches) -> Result<MlaVersion, Error> {
    // safe to use unwrap() because the option is required()
...
fn extract1(matches: &ArgMatches) -> Result<(), MlarError> {
...
fn repair1(matches: &ArgMatches) -> Result<(), MlarError> {
...
fn info1(matches: &ArgMatches) -> Result<(), MlarError> {
    // Safe to use unwrap() because the option is required()
    let mla_file = matches.get_one::<PathBuf>("input").unwrap();
    let path = Path::new(&mla_file);
    let mut file = File::open(path)?;
...
fn app() -> clap::Command {
    ...
    let input_args = vec![
        ...
        Arg::new("private_keys")
            .long("private_keys")
            .short('k')
            .help("Candidates ED25519 or mlakey private key paths (DER or PEM format)")
            .num_args(1)
    ...
    let output_args = vec![
        Arg::new("output")
            ...
            .required(true),
        Arg::new("public_keys")
            .help("ED25519 or mlakey public key paths (DER or PEM format)")
            .long("pubkey")
            .short('p')
            .num_args(1)
            .action(ArgAction::Append)
...
fn main() {
    ...
    // in order to address MLA 1 or MLA 2 functions accordingly
    let mla_version =
        get_mla_version_from_archive(&matches).map_err(|_| mla::errors::Error::UnsupportedVersion);
    ...
    let res = match mla_version {
        Ok(MlaVersion::V1) => matches.subcommand().map_or_else(
            ...
            |(cmd, matches)| match cmd {
                "extract" => extract1(matches),
                "repair" => repair1(matches),
                "info" => info1(matches),
                _ => {
